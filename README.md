...
- **多种输入**：视频（PyAV 流式解码）、连续编号图片序列、GIF、ICO（解码为从大到小的 PNG 清单）、空白 / 渐变序列
- **可视化裁剪**：序列级画面裁剪节点，在 1:1 预览图（框 = 素材像素尺寸，跟随图片）上直接拖拽裁剪框，支持纵横比锁定，可放在处理链任意位置
- **丰富处理节点**：时间 / 帧位截取、序列往复 / 抽帧 / 长度统一、序列相加 / 序列叠加（层叠合成，含「不缩放」策略）、帧冻结（首/末帧定格延长）、**序列剃刀**（胶片条拖拽剃刀线切割，实时预览切割处两侧帧）、亮度 / 饱和度 / 对比度 / 二值化 / 灰度化 / 反相、**色相/饱和度**（PS 式：全图或按目标色域/取色器，取代原「色相调整」）、旋转、纵横比挤压、**百分比缩放**（单序列输入，用户定义缩放 % + 缩放算法）、颜色量化（ImageMagick 原生）、帧差静止保持（录屏 → GIF 时域去噪）、通道分离 / 合并、超级键抠像、**颜色键抠像**（容差自控 + 羽化，滤色逻辑与超级键不同）、平移滚动动效
- **多种输出**：GIF 合成（wand 原样合成）、GIF 合成(FFmpeg)（PyAV 进程内 palettegen/paletteuse，编码时直接做帧优化）、GIF 合成(gifski)（每帧独立调色板 + 时域仿色，质量天花板，AGPL 随包 CLI）、GIF 优化（gifsicle）、WebP / APNG 动画导出（Pillow 内建）、ICO 合成、PNG 序列导出，均带「导出…」按钮直接保存
- **分析节点**：GIF 调色板、1:1 分辨率、GIF 优化形态、ICO 分辨率查看
- **自动模式**：修改参数或连线后自动重跑受影响链路；后台线程串行执行，可随时停止
- **缓存用量状态栏警告**：每次运行节点后统计缓存总用量，超过设置上限（自动清理无法压回）时状态栏显示**纯文本**警告（不使用 emoji/特殊符号——特殊字符会拖慢软件初始化）
- **自动保存**：设置管理器可设间隔（分钟，默认 10 开启、0 = 关闭）；方案有未保存更改时按间隔自动保存；正常退出自动清理自动保存文件，异常退出后下次启动弹窗提示恢复
...
- gifski（1.34.0，官方 Windows CLI，AGPL-3.0-or-later）（「GIF 合成(gifski)」节点使用；随包 `runtime/gifski/` 目录，运行 `scripts/prepare_gifski_runtime.py` 从官方 zip 解压）
...
cd 进入
```
将imagemagick与gifsicle（1.96）与gifski-cli分别复制到  
`src\gif_node_studio\runtime\gifsicle`   
与  
`src\gif_node_studio\runtime\imagemagick` 
与
`src\gif_node_studio\runtime\gifski` 
（或者参照`scripts\prepare_imagemagick_runtime.py`与`scripts\prepare_gifsicle_runtime.py`与`scripts/prepare_gifski_runtime.py`）
...
1. 从左侧节点库创建节点（按输入 / 预格式化 / 格式化 / 序列处理 / 画面处理 / 动效处理 / 通道处理 / 输出 / 分析分类）。
